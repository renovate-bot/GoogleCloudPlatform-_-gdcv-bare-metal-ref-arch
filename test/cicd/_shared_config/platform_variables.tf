...
locals {
  platform_terraform_bucket_name = var.platform_terraform_bucket_name != null ? var.platform_terraform_bucket_name : "${local.unique_identifier_prefix}-terraform-${local.platform_terraform_project_id}"
  platform_terraform_project_id  = var.platform_terraform_project_id != null ? var.platform_terraform_project_id : var.platform_default_project_id
...
variable "platform_default_project_id" {
  ...
  validation {
    ...
    error_message = "'platform_default_project_id' was not set, please set the value in '_shared_config/platform.auto.tfvars' file or via the TF_VAR_platform_default_project_id"
  }
}
...
variable "platform_name" {
  ...
  validation {
    condition     = length(var.platform_name) >= 1 && length(var.platform_name) <= 8
    error_message = "platform_name must be 1 - 8 characters."
  }
}
...
variable "platform_resource_name_prefix" {
  ...
  validation {
    condition     = length(var.platform_resource_name_prefix) >= 1 && length(var.platform_resource_name_prefix) <= 4
    error_message = "platform_resource_name_prefix must be 1 - 4 characters."
  }
}

variable "platform_terraform_bucket_name" {
  default     = null
  description = "The name of the Cloud Storage bucket to use for Terraform."
  type        = string
}

variable "platform_terraform_project_id" {
  default     = null
  description = "The project ID of the Terraform bucket."
  type        = string
}

variable "platform_terraform_write_tfvars" {
  default     = true
