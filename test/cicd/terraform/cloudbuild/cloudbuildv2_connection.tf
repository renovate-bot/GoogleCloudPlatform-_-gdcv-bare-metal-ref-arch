...
data "google_secret_manager_secret" "github_token" {
  ...
  secret_id = local.build_github_token_secret_name
}
...
resource "google_cloudbuildv2_repository" "gdc_solutions" {
  location          = local.build_location
  name              = "GoogleCloudPlatform-gdc-solutions"
  parent_connection = google_cloudbuildv2_connection.github.name
  project           = data.google_project.build.project_id
  remote_uri        = "https://github.com/GoogleCloudPlatform/gdc-solutions.git"
}
